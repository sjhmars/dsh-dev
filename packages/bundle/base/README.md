...
Out of the box, every profile built on this core provides: a DeepSeek model connection (the provider and model are configurable, and you can enable extra providers from your settings), the full tool set — file editing, shell commands, Exa-backed web search, subagents, task and goal tracking — durable sessions that survive restarts, and the default permission policy that confines file writes to your workspace and asks before risky actions. Exa search needs `EXA_API_KEY` in the launch environment but needs no profile patch or separate plugin installation. Telemetry stays off unless you opt in.
