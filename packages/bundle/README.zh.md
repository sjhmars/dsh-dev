---
description: "共享核心、浏览器 GUI、外部机器网关、一次性任务、ACP 与 SDK 应用表层的现成 dsh profile bundle。"
kind: "package-group"
---
...
本组列出 `dsh --profile` 使用的可安装 patch 层。每个包都声明 `dsh.bundle.patch`；启动器会叠放这些 patch 文档来组装具名 profile。`web`、`external-gateway`、`headless`、`acp` 与 `sdk` profile 以 `dsh-base` 为基础；External Gateway 还会先复用 Web 的 Host 组装，再移除浏览器传输。`sdk-minimal` 则由一个 bundle 提供完整配置树。领域包也可以在本目录之外声明附加层。
...
| [`external-gateway-app`](external-gateway-app/README.zh.md) | 覆盖在 Web Host 组装上的无头机器协议层 | 挂载隔离的 `/v1` 监听器 |
