...
export interface LaunchOptions {
  ...
  /**
   * Patch the shipped Exa search row to a deterministic endpoint and literal
   * fixture credential. Browser search scenarios keep the real provider while
   * avoiding external search traffic and ambient keys.
   */
  exaSearch?: {
    /** Exa-compatible base URL; the provider appends `/search`. */
    baseURL: string
    /** Fixture-only API key sent by the shipped search provider. */
    apiKey: string
  }
  /**
  ...
  const patches: PatchOptions[] = [
    ...
    ...options.exaSearch === undefined
      ...
      : [{
        id: 'web-search-exa',
        config: {
          apiKey: options.exaSearch.apiKey,
          baseURL: options.exaSearch.baseURL,
        },
      }],
...
function normalizeWebSessionVolatiles(log: string, workspaceCwd?: string): string {
  const normalizeValue = (value: unknown): unknown => {
    if (typeof value === 'string') {
      const anonymousUser = value.replace(
        /Anonymous user: [^.]+(?=\. Session sharing)/g,
        'Anonymous user: {{anonymousUserId}}',
      )
      return workspaceCwd === undefined ? anonymousUser : anonymousUser.split(workspaceCwd).join('{{cwd}}')
    }
...
async function assertReplaySession(
  ...
  expect(normalizeSessionSnapshots([normalizeWebSessionVolatiles(actual, sessionCwd)], actualContext)[0], `${fixturePath}: persisted replay`)
    .toBe(normalizeSessionSnapshots([normalizeWebSessionVolatiles(expected, expectedContext.cwd)], expectedContext)[0])
